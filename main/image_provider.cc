...
// if display support is present, initialise display buf
  ...
  if (ret != 0) {
    MicroPrintf("Falha ao inicializar camera\n");
    return kTfLiteError;
  }
  MicroPrintf(" Camera inicializada\n");
#else
  ESP_LOGE(TAG, "Camera nao suportada, verifique as configurações do menuconfig");
#endif
  return kTfLiteOk;
}
